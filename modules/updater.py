    ...
    def on_curl_script_exit(self, terminal, exit_status, user_data):
        ...
        if exit_status == 0:
            ...
            GLib.idle_add(
                ...
                f"Script terminó con status {exit_status}. Últimas líneas:\n{error_excerpt}",
            )
    ...
    if os.path.exists(snooze_file_path):
        try:
            ...
            if current_time - snooze_timestamp < SNOOZE_DURATION_SECONDS:
                ...
                print(
                    f"Chequeo pospuesto. Se reanudará después de {snooze_until_time_str}."
                )
                if is_standalone_mode and _QUIT_GTK_IF_NO_WINDOW_STANDALONE:
                    GLib.idle_add(Gtk.main_quit)
                return
                ...
                print(
                    "Periodo de snooze expirado. Eliminando archivo y chequeando actualizaciones."
                )
                os.remove(snooze_file_path)
        except ValueError:
            print(
                f"Error: contenido inválido en snooze file. Eliminando: {snooze_file_path}"
            )
            try:
                os.remove(snooze_file_path)
            ...
            print(
                f"Error procesando snooze file {snooze_file_path}: {e_snooze}. Procediendo con chequeo."
            )
            try:
                os.remove(snooze_file_path)
...
def run_updater():
    ...
    update_check_thread = threading.Thread(
        target=_initiate_update_check_flow, args=(True,), daemon=True
    )
